...
namespace brightness_histogram
{
    ...
    {
        public int[] new_color(int[] mas,int w)
        {
            int[] new_mas = new int[256];
            int s = 0;
            for (int i = 0; i < mas.Length; i++)
            {
                s += mas[i];
                new_mas[i] = (int)255*s/w;
            }
            return new_mas;
        }
        public void histogram(Bitmap my_img, int xx, int yy, int ww, int hh)
        {
        ...
        public Form1()
        {
            ...
            histogram(my_img, 0,0,500,250);

            int w = my_img.Width * my_img.Height;

            int[] new_r = new_color(bright_r,w);
            int[] new_g = new_color(bright_g,w);
            int[] new_b = new_color(bright_b,w);
            ...
            histogram(new_img, 0, 250, 500, 250);
